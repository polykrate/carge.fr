...
export const Workflows = () => {
  ...
  const PINNED_WORKFLOWS = [
    config.EXAMPLES.WORKFLOW.ragHash, // Macallan 25 Years Supply Chain (7 steps: Production → Consumer)
  ...
  const handleToggleFavorite = (workflowHash) => {
    if (!selectedAccount) {
      showError(t('messages.walletNotConnectedFavorites'));
      return;
    }
    ...
    if (newStatus) {
      ...
      showSuccess(t('messages.removedFromFavorites'));
  ...
  const searchByTags = async () => {
    try {
      ...
      console.log('Found workflows:', ragMasters.map(r => ({ hash: r.hash, name: r.metadata.name, stepsCount: r.metadata.steps.length })));
      setDisplayRags(ragMasters);
  ...
  const submitWorkflow = async (e) => {
    ...
    if (!selectedAccount) {
      showError(t('messages.walletNotConnectedWorkflow'));
      return;
    }
          ...
          <div className="bg-gradient-to-br from-blue-50 to-white border-2 border-blue-200 rounded-xl p-6 mb-6 shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <input
                ...
                placeholder="e.g., supply-chain master generic v1"
                autoCapitalize="off"
                autoComplete="off"
                autoCorrect="off"
                spellCheck="false"
            ...
            <div className="mt-3 flex items-start gap-2 text-xs text-gray-500">
              ...
              <span>
                {t('workflows.pinnedWorkflowsInfo')}
              </span>
            </div>
          </div>
